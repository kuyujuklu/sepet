...
const OrderCardForAdmin = ({ order, hasArrow = true }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-x-20">
        <div className="font-bold">
          {t("Has courier: ")} {order?.courier_info?.is_reserved ? "true" : "false"}
        </div>
        <div className="">
          {
            order?.courier_info?.is_reserved &&
            <>
              {t("Courier ID: ")} {order?.courier_info?.reserver_courier_id}
            </>
          }
        </div>
      </div>

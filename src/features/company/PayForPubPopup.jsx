...
import { closePayForPubPopup, selectPayForPubPopupState } from "./companySlice";
import { useCallback } from "react";
import Popup from "@/components/Popup/Popup";

const PayForPubPopup = () => {
    const { t } = useTranslation();

    const dispatch = useDispatch();
    const popupState = useSelector(selectPayForPubPopupState);

    const closePopup = useCallback(() => {
        dispatch(closePayForPubPopup());
    }, [dispatch]);
            ...
            <div className="py-4">
                <header className="mb-8">
                    <h1 className="font-bold text-center text-2xl text-gray-800">
                        {t("admin.popups.pay_for_pub_popup.header")}
                    </h1>
                </header>

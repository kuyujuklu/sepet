...
export const basketSlice = createSlice({
  ...
  reducers: {
    ...
    closeClearBasketPopup(state) {
      ...
      state.clearBasketPopup.text = "Are you sure you want to clear your basket?";
    ...
    increaseDish(state, action) {
      ...
      if(state.pubID && state.pubID !== pubID) {
        state.clearBasketPopup.isOpened = true;
        state.clearBasketPopup.text = "Are you sure you want to add a dish from another pub? Your current basket will be cleared."
      ...
      if(!state.basket[id]) state.basket[id] = { count: 0, price: action.payload.price};
...
export const { increaseDish, decreaseDish, openClearBasketPopup, closeClearBasketPopup, doClearPopupConfirmingAction } = basketSlice.actions;

export const selectBasket = (state) => state.basket.basket;
export const selectBasketPubID = (state) => state.basket.pubID;
export const selectDishFromBasket = (id) => (state) => state.basket.basket[id] || null;

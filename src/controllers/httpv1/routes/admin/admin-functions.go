...
func (c *adminController) GetPubRefreshToken(ctx *fiber.Ctx) error {
	...
	company := pub.Company
	fmt.Println("Company: ", helpers.ConvertToJSON(company))
...
type CreateTelegramSuperUserInput struct {
	Username string `json:"username"`
}

func (c *adminController) CreateTelegramSuperUser(ctx *fiber.Ctx) error {
	_, userSignificance, _, err := h.GetUserIDSignificanceAndRoleFromLocals(ctx)
	if err != nil {
		return h.SendError(ctx, err, h.AUTOMATIC_STATUS_CODE)
	}
	...
	input, validationErrors, err := input.ParseRequestBody[CreateTelegramSuperUserInput](ctx)
	if err != nil {
		return h.SendError(ctx, err, h.AUTOMATIC_STATUS_CODE)
	}
	if len(validationErrors) > 0 {
	...
	_, err = c.TelegramService.CreateTelegramSuperUser(input.Username)
	if err != nil {
		return h.SendError(ctx, err, h.AUTOMATIC_STATUS_CODE)
	}
...
type SetPubsForSuperUserInput struct {
	Username string `json:"username"`
	PubsJSON string `json:"pubs_json"`
}

func (c *adminController) SetPubsForSuperUser(ctx *fiber.Ctx) error {
	_, userSignificance, _, err := h.GetUserIDSignificanceAndRoleFromLocals(ctx)
	if err != nil {
		return h.SendError(ctx, err, h.AUTOMATIC_STATUS_CODE)
	}
	...
	input, validationErrors, err := input.ParseRequestBody[SetPubsForSuperUserInput](ctx)
	if err != nil {
		return h.SendError(ctx, err, h.AUTOMATIC_STATUS_CODE)
	}
	if len(validationErrors) > 0 {
	...
	err = c.TelegramService.SetPubsForSuperUser(input.Username, input.PubsJSON)
	if err != nil {
		return h.SendError(ctx, err, h.AUTOMATIC_STATUS_CODE)
	}

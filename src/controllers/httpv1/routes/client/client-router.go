...
func (c *clientController) UnauthorizedRouter(router fiber.Router) {
	...
	router.Post("/registration", c.CreateRegistrationSession)
	router.Post("/registration/validation", c.ValidateRegistration)
	router.Post("/authentication", c.CreateAuthenticationSession)
	router.Post("/authentication/refresh-token", c.RefreshToken)
	router.Post("/authentication/validation", c.ValidateAuthentication)
